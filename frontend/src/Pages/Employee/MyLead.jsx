...
function MyLead() {
  ...
  const [loading, setLoading] = useState(false);
  const [searchText, setSearchText] = useState("");
  const [activeFilter, setActiveFilter] = useState("all"); // all / installation / service / om
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingLead, setEditingLead] = useState(null);
  ...
  // Apply filters whenever activeFilter or searchText changes
  useEffect(() => {
    let result = [...leads];

    // 1. Enquiry Type filter
    if (activeFilter !== "all") {
      result = result.filter((lead) => lead.enquiryType === activeFilter);
    }
    ...
    <div style={{ padding: "24px", background: "#fff", minHeight: "100%" }}>
      ...
      >
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <Button
            type={
              activeFilter === "New Solar Power Plant Installation"
                ? "primary"
                : "default"
            }
            onClick={() =>
              handleFilterClick("New Solar Power Plant Installation")
            }
            style={
              activeFilter === "New Solar Power Plant Installation"
                ? activeButtonStyle
          ...
          <Button
            type={
              activeFilter === "Solar Power Plant Service"
                ? "primary"
                : "default"
            }
            onClick={() => handleFilterClick("Solar Power Plant Service")}
            style={
              activeFilter === "Solar Power Plant Service"
                ? activeButtonStyle
                : buttonStyle
          ...
          <Button
            type={
              activeFilter === "Operation & Maintenance Service"
                ? "primary"
                : "default"
            }
            onClick={() => handleFilterClick("Operation & Maintenance Service")}
            style={
              activeFilter === "Operation & Maintenance Service"
                ? activeButtonStyle
                : buttonStyle
...
// Simple button styles (you can move to CSS file later)
const buttonStyle = {
  transition: "all 0.2s",
};
